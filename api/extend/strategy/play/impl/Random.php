 ...
 * 全随机
 */
class Random implements PlayInterface
{
    ...
    public function award($param)
    {
        ...
        // 查询所有的参与人
        $joinUsers = (new Order())->field('user_id,user_name')->where('blindbox_id', $data['blindbox_id'])
            ->where('box_id', $data['box_id'])->select()->toArray();
        ...
        foreach ($goodsList as $vo) {
            // 全局随机赠送
            $key = array_rand($joinUsers);
            $awardUser = $joinUsers[$key];
