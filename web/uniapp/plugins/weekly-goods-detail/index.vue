	...
	<!-- <app-layout v-if="refreshPage"> -->
		...
		<view class="container" :style="{'padding-top': phoneHieght}">
			...
			<scroll-view class="body" enableFlex scroll-y>
				<view class="goods-swiper">
					<view class="goods-background main-center-flex">
						<!-- #ifdef APP-PLUS || H5 -->
						<img :src="baseUrl.imgroot+'/static/images/back_2.png'" alt="" class="back-img">
						<!-- #endif -->
						<!-- #ifdef MP-WEIXIN -->
						<image :src="baseUrl.imgroot+'/static/images/back_2.png'" mode="" class="back-img"></image>
						<!-- #endif -->
					</view>
			...
			<view class="buy-button main-center-flex">
				<view class="buy-button-detail" :style="{'background-image': 'url('+WeeklyPayBtn[5]+')'}"
					@tap="goPayPage">
					<view class="limit-class">{{ '限购' + buy_limit_num + '次' }}</view>
					<view class="day-class">{{ WeeklyTitle2[curDay] }}</view>
					<view class="main-center-flex">
						<view class="btn-title">立即开盒</view>
						<view class="pay-title">{{ payTitle }}</view>
						<!-- :style="{background: weeklyTitleBgColor[curDay], color: weeklyTitleColor[curDay]}" -->
					</view>
					<view class="main-center-flex">
						<view>¥{{price}}</view>
					</view>
			...
			<u-popup :show="orderVisible" mode="bottom" @close="closeOrderPopup">
				<weekly-order-sure ref="order" class="order-view" @closeOrder="closeVisible">
				</weekly-order-sure>
			</u-popup>
...
<script>
	...
	import {weekTrail} from '@/api/newer.js';
	import WeeklyOrderSure from '@/components/weekly-order-sure/index.vue'
	import GoodsDetail from '@/components/goods-detail/index.vue';
	import GoodsShare from '@/components/goods-detail/goods-share.vue';
	import BarrageBar from '@/components/winning-barrage/barrage-bar.vue';
	import { GoodsRightMenu, HonorListImg, WeeklyPayBtn, WeeklyTitle2 } from '../../utils/objectValue';
	...
	export default {
		...
		data() {
			return {
				...
				weeklyInfo: {},
				weeklyTitleBgColor: {
					1: 'linear-gradient(180deg, rgba(110,127,255,0.62) 0%, #FFFFFF 100%);',
					2: 'linear-gradient(180deg, rgba(255,91,137,0.62) 0%, #FFFFFF 100%);',
					3: 'linear-gradient(180deg, rgba(110,127,255,0.62) 0%, #FFFFFF 100%);',
					4: 'linear-gradient(180deg, rgba(255,91,137,0.62) 0%, #FFFFFF 100%);', 
					5: 'linear-gradient(180deg, rgba(255,255,255,0.62) 0%, #FFFFFF 100%);',
					6: 'linear-gradient(180deg, rgba(255,255,255,0.62) 0%, #FFFFFF 100%);',
					7: 'linear-gradient(180deg, rgba(255,255,255,0.62) 0%, #FFFFFF 100%);'
				},
				weeklyTitleColor: {
					1: '#7795FF',
					2: '#FF649C',
		...
		async onLoad(params) {
			...
			if(scene) {
				...
				this.blindbox_id = arr[1].substring(4);
				uni.setStorageSync('agent_user_id', arr[0].substring(5));
			} else {
				// 列表跳转进入
				this.blindbox_id = params.blindId;
			}

			const {id, day} = params;
			this.weeklyInfo = this.$store.getters['goods/getWeeklyDetail'];
			const {buy_limit_num, title} = this.weeklyInfo;
			this.buy_limit_num = buy_limit_num;
			this.payTitle = title;
			this.itemId = id;
			this.curDay = day;
		...
		methods: {
			...
			async getBlindDetail() {
				uni.showLoading({
					title: '',
					mask: true
				});
				const {code, data} = await get_blind_detail({id: this.blindbox_id});
				uni.hideLoading();
				if (code == 0) {
			...
			async goPayPage() {
				...
				const {code, data, msg = ''} = await weekTrail({id: this.itemId});
				uni.hideLoading();
				if(code == 0) {
					// 可以购买
					const params = {
...
<style lang="scss" scoped>
	...
	.buy-button-detail {
		...
		.limit-class{
			...
			height: 46rpx;
			/* padding: 4rpx; */
			background: linear-gradient(to bottom, #FFD130 30%, #FFFDF5 99%);
			color: #7A4A2F;
			font-size: 30rpx;
			text-align: center;
			line-height: 46rpx;
		...
		.pay-title{
			margin-left: 10rpx;
			color: #8C64FF;
			background: linear-gradient(to top, rgba(255,255,255,0.62) 0%, #FFFFFF 100%);
			font-size: 22rpx;
