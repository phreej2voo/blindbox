<template>
	...
	<view class="container">
		...
		<scroll-view v-if="showPayInfo" class="scroll-view" enableFlex scroll-y>
			<view class="weekly-bg" :style="{'background-image': 'url('+WeeklyOrderSureImg[curDay]+')'}">
				<view class="num-title">{{ payInfo.num + '发保底，必出好货' }}</view>
				<view class="day-title">{{ WeeklyTitle[curDay] }}</view>
			</view>
			<view class="pay-info">
				<view class="pay-info_1 main-start-flex">
					<image :src="payInfo.info.img" class="goods-img"></image>
					<view class="pay-info-detail_1">
						<view class="main-space-between">
							<view class="pay-info-name text-ellipsis">
								<view class="normal_title">{{payInfo.info.title}}</view>
								<view class="sub_title">{{ payInfo.info.sub_title }}</view>
							</view>
...
<script>
	...
	import { createWeekOrder } from '@/api/newer.js';
	import { HonorListImg,WeeklyTitle,WeeklyOrderSureImg } from '../../utils/objectValue';
	...
	export default {
		...
		methods: {
			...
			initData(params) {
				...
				const {blindbox_id, id, curDay} = param;
			...
			async createOrder(){
				...
				const params = {id: this.itemId, pay_way: this.currentPayType};
				const {code, data} = await createWeekOrder(params);
				if (code == 0) {
					const {order_no = ''} = data;
...
<style lang="scss" scoped>
	...
	.pay-info-name {
		...
		.sub_title{
			padding: 4rpx 6rpx;
			background: linear-gradient(109deg, #FF6636 0%, #FFCC7E 100%);
			opacity: 1;
			font-size: 22rpx;
