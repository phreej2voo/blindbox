<template>
	<view class="container" @touchmove.stop.prevent="moveHandle" :style="{'background-image': GoodsPoolImg.back}">
		...
		<!-- :style="{'background-image': (isShowPropList ? 'url('+RedrawImg+')' : '')}" -->
		<view class="contents">
			...
			<!-- <view v-if="isShowPropList" class="new-redraw-content main-space-around">
				<view class="redraw-text">{{ `还有${propList.length}张重抽卡` }}</view>
		...
		<!-- 重抽卡 初始页面显示 后续重抽不显示 -->
		<!-- <view v-if="isFirstShowPropNum && !isShowPropList" class="redraw-content main-space-around" :style="{'background-image': 'url('+RedrawImg+')'}">
...
<script>
	...
	import { RedrawImg, CouponsImgObj, GoodsPoolImg } from '../../utils/objectValue';

	export default {
		props: ['goods_list', 'colorObj', 'propList', 'prop_goods_list', 'jackpot'],
		data() {
			return {
				total: 0,
				common_color: '',
		...
		computed: {
			// 显示新道具卡列表
			isShowPropList() {
				return this.prop_goods_list && this.prop_goods_list.length;
			},
			isFirstShowPropNum() {
